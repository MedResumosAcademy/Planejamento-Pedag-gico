...
export async function getGravacoesByProfessor(professorId: string): Promise<Gravacao[]> {
  ...
  const { data, error } = await supabase
    .from('gravacoes')
    .select('*, disciplinas(nome, cor), temas(tema_especifico)')
...
export async function createGravacao(payload: {
  professor_id: string; disciplina_id: number; tema_id: number
  data_hora: string; duracao_minutos: number; observacoes?: string
}): Promise<Gravacao> {
  const supabase = createClient()
  const { data, error } = await supabase
    .from('gravacoes')
    .insert({ ...payload, status: 'proposta', gravada: false, cancelada: false, links: [] })
    .select().single()
  if (error) throw error
  return data
}
...
export async function updateGravacao(id: number, updates: {
  ...
  links?: ExternalLink[]; concluida_em?: string | null
}): Promise<void> {
  const supabase = createClient()
