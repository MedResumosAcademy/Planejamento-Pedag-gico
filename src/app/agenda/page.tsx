...
const STATUS_COLOR: Record<string, string> = { proposta: '#d97706', aprovada: '#2563eb', concluida: '#16a34a', cancelada: '#dc2626' }
...
export default function AgendaPage() {
  ...
  const inp = { width: '100%', background: 'rgba(0,0,0,0.05)', border: '1px solid rgba(0,0,0,0.1)', borderRadius: 8, padding: '10px 12px', color: '#1e293b', fontSize: 13, outline: 'none', boxSizing: 'border-box' as const }
    ...
    <div style={{ minHeight: '100vh', background: '#f4f6fb', padding: '32px 40px' }}>
      <div style={{ maxWidth: 1200, margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
          ...
          <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            ...
            <button onClick={() => setWeekOffset(0)} style={{ background: 'rgba(0,0,0,0.05)', border: '1px solid rgba(0,0,0,0.1)', borderRadius: 8, padding: '8px 12px', color: '#475569', cursor: 'pointer', fontSize: 11 }}>Today</button>
            <button onClick={() => setWeekOffset(w => w + 1)} style={{ background: 'rgba(0,0,0,0.05)', border: '1px solid rgba(0,0,0,0.1)', borderRadius: 8, padding: '8px 14px', color: '#475569', cursor: 'pointer' }}>→</button>
        ...
        <div style={{ background: 'rgba(0,0,0,0.02)', border: '1px solid rgba(0,0,0,0.06)', borderRadius: 16, overflow: 'hidden', marginBottom: 24 }}>
          <div style={{ display: 'grid', gridTemplateColumns: '90px repeat(7, 1fr)', borderBottom: '1px solid rgba(0,0,0,0.06)' }}>
            <div style={{ padding: '12px 8px', borderRight: '1px solid rgba(0,0,0,0.04)' }} />
            {weekDates.map((date, i) => {
              const isToday = date.toISOString().split('T')[0] === new Date().toISOString().split('T')[0]
              return (
                <div key={i} style={{ padding: '12px 8px', textAlign: 'center', borderRight: i < 6 ? '1px solid rgba(0,0,0,0.04)' : 'none', background: isToday ? 'rgba(99,102,241,0.08)' : 'transparent' }}>
                  <div style={{ fontSize: 10, color: '#64748b', fontWeight: 600, textTransform: 'uppercase' }}>{DAYS[date.getDay()]}</div>
                  <div style={{ fontSize: 20, fontWeight: 700, color: isToday ? '#7c3aed' : '#1e293b' }}>{date.getDate()}</div>
                  <div style={{ fontSize: 10, color: '#94a3b8' }}>{date.toLocaleDateString('pt-BR', { month: 'short' })}</div>
                </div>
              )
            })}
          </div>
          {periods.map((period, pi) => (
            <div key={period.key} style={{ display: 'grid', gridTemplateColumns: '90px repeat(7, 1fr)', borderBottom: pi < 2 ? '1px solid rgba(0,0,0,0.06)' : 'none' }}>
              <div style={{ padding: '16px 8px', borderRight: '1px solid rgba(0,0,0,0.04)', display: 'flex', flexDirection: 'column', gap: 2 }}>
                <div style={{ fontSize: 11, color: '#475569', fontWeight: 600 }}>{period.label}</div>
                <div style={{ fontSize: 9, color: '#94a3b8' }}>{period.range}</div>
              </div>
              {weekDates.map((date, di) => {
                ...
                return (
                  <div key={di} style={{ minHeight: 100, padding: 6, borderRight: di < 6 ? '1px solid rgba(0,0,0,0.04)' : 'none', background: isToday ? 'rgba(99,102,241,0.02)' : 'transparent', display: 'flex', flexDirection: 'column', gap: 4 }}>
                    {cellGravs.map(g => {
                      ...
                      return (
                        <div key={g.id} onClick={() => canOpen && setSelectedGravacao(g)}
                          ...
                          <div style={{ fontSize: 10, color: '#334155', fontWeight: 600, marginTop: 2 }}>{g.disciplinas?.nome?.slice(0, 16)}{(g.disciplinas?.nome?.length || 0) > 16 ? '…' : ''}</div>
                          <div style={{ fontSize: 9, color: '#64748b', marginTop: 1 }}>{g.temas?.tema_especifico?.slice(0, 22)}{(g.temas?.tema_especifico?.length || 0) > 22 ? '…' : ''}</div>
                          {g.links?.length > 0 && <div style={{ fontSize: 9, color: '#2563eb', marginTop: 2 }}>🔗 {g.links.length}</div>}
                          {isCoordinator && g.status === 'proposta' && (
                            <div style={{ display: 'flex', gap: 3, marginTop: 4 }}>
                              <button onClick={e => { e.stopPropagation(); quickApprove(g.id, 'aprovada') }} style={{ fontSize: 8, background: 'rgba(96,165,250,0.2)', border: 'none', borderRadius: 3, padding: '2px 5px', color: '#2563eb', cursor: 'pointer', fontWeight: 700 }}>✓</button>
                              <button onClick={e => { e.stopPropagation(); quickApprove(g.id, 'cancelada') }} style={{ fontSize: 8, background: 'rgba(248,113,113,0.2)', border: 'none', borderRadius: 3, padding: '2px 5px', color: '#dc2626', cursor: 'pointer', fontWeight: 700 }}>✕</button>
                            </div>
                          )}
                        </div>
                      )
          ...
          <div>
            ...
            {gravacoes.filter(g => g.status === 'proposta').map(g => (
              <div key={g.id} style={{ background: 'rgba(251,191,36,0.05)', border: '1px solid rgba(251,191,36,0.2)', borderRadius: 12, padding: '14px 18px', display: 'flex', alignItems: 'center', gap: 16, marginBottom: 8 }}>
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 13, fontWeight: 600, color: '#1e293b' }}>{g.colaboradores?.nome}</div>
                  <div style={{ fontSize: 12, color: '#64748b' }}>{g.disciplinas?.nome} · {g.temas?.tema_especifico}</div>
                  <div style={{ fontSize: 11, color: '#d97706', marginTop: 2 }}>{new Date(g.data_hora).toLocaleString('pt-BR')} · {g.duracao_minutos} min</div>
                </div>
                <div style={{ display: 'flex', gap: 8 }}>
                  <button onClick={() => setSelectedGravacao(g)} style={{ background: 'rgba(0,0,0,0.05)', border: '1px solid rgba(0,0,0,0.1)', borderRadius: 8, padding: '8px 14px', color: '#475569', fontSize: 12, cursor: 'pointer' }}>Details</button>
                  <button onClick={() => quickApprove(g.id, 'aprovada')} style={{ background: 'rgba(96,165,250,0.15)', border: '1px solid rgba(96,165,250,0.3)', borderRadius: 8, padding: '8px 16px', color: '#2563eb', fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>✓ Approve</button>
                  <button onClick={() => quickApprove(g.id, 'cancelada')} style={{ background: 'rgba(248,113,113,0.1)', border: '1px solid rgba(248,113,113,0.2)', borderRadius: 8, padding: '8px 14px', color: '#dc2626', fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>✕ Reject</button>
                </div>
              </div>
            ))}
          </div>
      ...
      {showForm && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.75)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 50, padding: 20 }}>
          <div style={{ background: '#ffffff', border: '1px solid rgba(0,0,0,0.1)', borderRadius: 20, padding: 32, width: '100%', maxWidth: 480 }}>
            <h2 style={{ fontSize: 18, fontWeight: 700, color: '#0f172a', marginBottom: 24 }}>Propose Recording</h2>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div>
                <label style={{ fontSize: 11, color: '#475569', fontWeight: 600, display: 'block', marginBottom: 6, textTransform: 'uppercase' }}>Discipline</label>
              ...
              <div>
                <label style={{ fontSize: 11, color: '#475569', fontWeight: 600, display: 'block', marginBottom: 6, textTransform: 'uppercase' }}>Topic</label>
                <select value={form.tema_id} onChange={e => setForm(f => ({ ...f, tema_id: Number(e.target.value) }))} disabled={!form.disciplina_id} style={{ ...inp, color: form.disciplina_id ? '#1e293b' : '#94a3b8' }}>
                  <option value={0}>Select...</option>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
                <div>
                  <label style={{ fontSize: 11, color: '#475569', fontWeight: 600, display: 'block', marginBottom: 6, textTransform: 'uppercase' }}>Date</label>
                  <input type="date" value={form.data} onChange={e => setForm(f => ({ ...f, data: e.target.value }))} style={inp} />
                </div>
                <div>
                  <label style={{ fontSize: 11, color: '#475569', fontWeight: 600, display: 'block', marginBottom: 6, textTransform: 'uppercase' }}>Time</label>
                  <input type="time" value={form.hora} onChange={e => setForm(f => ({ ...f, hora: e.target.value }))} style={inp} />
                </div>
                <div>
                  <label style={{ fontSize: 11, color: '#475569', fontWeight: 600, display: 'block', marginBottom: 6, textTransform: 'uppercase' }}>Min</label>
              ...
              <div>
                <label style={{ fontSize: 11, color: '#475569', fontWeight: 600, display: 'block', marginBottom: 6, textTransform: 'uppercase' }}>Notes</label>
            ...
            <div style={{ display: 'flex', gap: 10, marginTop: 24 }}>
              <button onClick={() => setShowForm(false)} style={{ flex: 1, background: 'rgba(0,0,0,0.05)', border: '1px solid rgba(0,0,0,0.1)', borderRadius: 10, padding: '11px', color: '#475569', fontSize: 13, cursor: 'pointer' }}>Cancel</button>
