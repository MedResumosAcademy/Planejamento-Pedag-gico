    ...
    <div style={{ minHeight:'100vh', background:'#f4f6fb', padding:'32px 40px' }}>
      <div style={{ maxWidth:960, margin:'0 auto' }}>
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:24 }}>
          <div>
            <h1 style={{ fontSize:24, fontWeight:700, color:'#0f172a', margin:0 }}>Collaborators</h1>
          ...
          <div style={{ display:'flex', gap:10 }}>
            <button onClick={() => setShowInviteForm(true)} style={{ background:'rgba(99,102,241,0.15)', border:'1px solid rgba(99,102,241,0.3)', borderRadius:10, padding:'10px 18px', color:'#7c3aed', fontSize:13, fontWeight:600, cursor:'pointer' }}>
        ...
        <div style={{ background:'rgba(8,145,178,0.08)', border:'1px solid rgba(8,145,178,0.2)', borderRadius:14, padding:'16px 20px', marginBottom:28, display:'flex', alignItems:'center', justifyContent:'space-between' }}>
          <div>
            <div style={{ fontSize:13, fontWeight:600, color:'#0891b2', marginBottom:4 }}>📎 Professor Registration Link</div>
          ...
          <button onClick={() => { navigator.clipboard.writeText(window.location.origin + '/cadastro/professor'); alert('Link copied!') }}
            style={{ background:'rgba(8,145,178,0.15)', border:'1px solid rgba(8,145,178,0.3)', borderRadius:8, padding:'8px 16px', color:'#0891b2', fontSize:12, fontWeight:600, cursor:'pointer', whiteSpace:'nowrap' }}>
            Copy Link 📋
          </button>
        </div>
        ...
        {loading ? (
          ...
          <>
            ...
            {coordinators.length > 0 && (
              <div style={{ marginBottom:28 }}>
                ...
                <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
                  {coordinators.map(c => (
                    <div key={c.id} style={{ background:'rgba(0,0,0,0.03)', border:'1px solid rgba(0,0,0,0.06)', borderRadius:12, padding:'14px 18px', display:'flex', alignItems:'center', gap:14 }}>
                      ...
                      <div style={{ background:'rgba(99,102,241,0.15)', borderRadius:6, padding:'4px 10px', fontSize:11, fontWeight:600, color:'#7c3aed' }}>Coordinator</div>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div>
              ...
              {professors.length === 0 ? (
                <div style={{ background:'rgba(0,0,0,0.02)', border:'1px dashed rgba(0,0,0,0.06)', borderRadius:12, padding:40, textAlign:'center', color:'#94a3b8' }}>
                ...
                <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
                  {professors.map(c => {
                    ...
                    return (
                      <div key={c.id} style={{ background:'rgba(0,0,0,0.03)', border:'1px solid rgba(0,0,0,0.06)', borderRadius:12, overflow:'hidden' }}>
                        <div style={{ padding:'14px 18px', display:'flex', alignItems:'center', gap:14 }}>
                          ...
                          <button onClick={() => router.push(`/admin/colaboradores/${c.id}`)}
                            style={{ background:'rgba(8,145,178,0.1)', border:'1px solid rgba(8,145,178,0.2)', borderRadius:8, padding:'6px 12px', color:'#0891b2', fontSize:11, fontWeight:600, cursor:'pointer', marginRight:8 }}>
                            View Profile →
                          </button>
                          <button onClick={() => setExpanded(isExpanded ? null : c.id)}
                            style={{ background:'rgba(0,0,0,0.04)', border:'1px solid rgba(0,0,0,0.08)', borderRadius:8, padding:'6px 10px', color:'#64748b', fontSize:11, cursor:'pointer' }}>
                            {isExpanded ? '▲' : '▼'}
                          </button>
                        </div>
                        {isExpanded && (
                              ...
                              <div>
                                ...
                                <div style={{ display:'flex', flexWrap:'wrap', gap:6 }}>
                                  {discNames.length > 0 ? discNames.map(n => (
                                    <span key={n as string} style={{ background:'rgba(8,145,178,0.1)', border:'1px solid rgba(8,145,178,0.2)', borderRadius:6, padding:'4px 10px', fontSize:11, color:'#0891b2' }}>{n}</span>
                                  )) : <span style={{ fontSize:12, color:'#94a3b8' }}>No disciplines</span>}
                                </div>
                              </div>
                                ...
                                <div>
                                  ...
                                  <div style={{ display:'flex', flexWrap:'wrap', gap:6 }}>
                                    {c.professor_horarios.map((h, i) => (
                                      <span key={i} style={{ background:'rgba(0,0,0,0.04)', border:'1px solid rgba(0,0,0,0.08)', borderRadius:6, padding:'4px 10px', fontSize:11, color:'#475569' }}>
        ...
        {showInviteForm && (
          <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.7)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:50 }}>
            <div style={{ background:'#ffffff', border:'1px solid rgba(0,0,0,0.1)', borderRadius:20, padding:32, width:420 }}>
              <h2 style={{ fontSize:18, fontWeight:700, color:'#0f172a', marginBottom:24 }}>Invite Coordinator</h2>
              <div style={{ marginBottom:16 }}>
                <label style={{ fontSize:11, color:'#475569', fontWeight:500, display:'block', marginBottom:6 }}>FULL NAME</label>
                <input value={inviteName} onChange={e => setInviteName(e.target.value)} placeholder="Full name"
                  style={{ width:'100%', background:'rgba(0,0,0,0.05)', border:'1px solid rgba(0,0,0,0.1)', borderRadius:8, padding:'9px 12px', color:'#1e293b', fontSize:13, outline:'none', boxSizing:'border-box' as const }} />
              </div>
              <div style={{ marginBottom:24 }}>
                <label style={{ fontSize:11, color:'#475569', fontWeight:500, display:'block', marginBottom:6 }}>E-MAIL</label>
              ...
              <div style={{ display:'flex', gap:10 }}>
                <button onClick={() => { setShowInviteForm(false); setError('') }} style={{ flex:1, background:'rgba(0,0,0,0.05)', border:'1px solid rgba(0,0,0,0.1)', borderRadius:10, padding:'10px', color:'#475569', fontSize:13, cursor:'pointer' }}>Cancel</button>
        ...
        {showForm && (
          <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.7)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:50, padding:20 }}>
            <div style={{ background:'#ffffff', border:'1px solid rgba(0,0,0,0.1)', borderRadius:20, padding:32, width:'100%', maxWidth:560, maxHeight:'90vh', overflowY:'auto' }}>
              <h2 style={{ fontSize:18, fontWeight:700, color:'#0f172a', marginBottom:24 }}>New Collaborator</h2>
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16, marginBottom:16 }}>
                {[{label:'FULL NAME',key:'nome',type:'text',ph:'Full name'},{label:'E-MAIL',key:'email',type:'email',ph:'email@example.com'},{label:'PHONE',key:'telefone',type:'tel',ph:'(00) 00000-0000'},{label:'PASSWORD',key:'password',type:'password',ph:'••••••••'}].map(f => (
                  <div key={f.key}>
                    <label style={{ fontSize:11, color:'#475569', fontWeight:500, display:'block', marginBottom:6 }}>{f.label}</label>
              ...
              <div style={{ marginBottom:20 }}>
                <label style={{ fontSize:11, color:'#475569', fontWeight:500, display:'block', marginBottom:8 }}>ROLE</label>
                <div style={{ display:'flex', gap:8 }}>
                  {(['professor','coordenador'] as const).map(n => (
                    <button key={n} onClick={() => setForm(f => ({ ...f, nivel: n }))}
                      style={{ flex:1, padding:'9px', borderRadius:8, border:'1px solid', borderColor: form.nivel===n ? '#6366f1' : 'rgba(0,0,0,0.1)', background: form.nivel===n ? 'rgba(99,102,241,0.15)' : 'transparent', color: form.nivel===n ? '#7c3aed' : '#64748b', fontSize:13, fontWeight:600, cursor:'pointer' }}>
                ...
                <>
                  ...
                  <div style={{ marginBottom:20 }}>
                    <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:8 }}>
                      <label style={{ fontSize:11, color:'#475569', fontWeight:500 }}>AVAILABILITY</label>
                      <button onClick={addHorario} style={{ fontSize:11, color:'#7c3aed', background:'none', border:'none', cursor:'pointer', fontWeight:600 }}>+ Add slot</button>
                    </div>
                    {form.horarios.map((h, i) => (
                      <div key={i} style={{ display:'flex', gap:8, marginBottom:8, alignItems:'center' }}>
                        <select value={h.dia_semana} onChange={e => updateHorario(i, 'dia_semana', Number(e.target.value))}
                          style={{ background:'rgba(0,0,0,0.05)', border:'1px solid rgba(0,0,0,0.1)', borderRadius:8, padding:'8px', color:'#1e293b', fontSize:12, outline:'none' }}>
                        ...
                        <input type="time" value={h.hora_fim} onChange={e => updateHorario(i, 'hora_fim', e.target.value)}
                          style={{ background:'rgba(0,0,0,0.05)', border:'1px solid rgba(0,0,0,0.1)', borderRadius:8, padding:'8px', color:'#1e293b', fontSize:12, outline:'none' }} />
                        <button onClick={() => removeHorario(i)} style={{ background:'rgba(239,68,68,0.1)', border:'none', borderRadius:6, padding:'6px 10px', color:'#dc2626', cursor:'pointer', fontSize:12 }}>✕</button>
                      </div>
                    ))}
                  </div>
                </>
              ...
              <div style={{ display:'flex', gap:10, justifyContent:'flex-end' }}>
                <button onClick={() => { setShowForm(false); setError('') }} style={{ background:'rgba(0,0,0,0.05)', border:'1px solid rgba(0,0,0,0.1)', borderRadius:10, padding:'10px 20px', color:'#475569', fontSize:13, cursor:'pointer' }}>Cancel</button>
