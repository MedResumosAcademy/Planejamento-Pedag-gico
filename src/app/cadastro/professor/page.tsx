...
export default function CadastroProfessor() {
  ...
  const inputStyle = { width:'100%', background:'rgba(0,0,0,0.05)', border:'1px solid rgba(0,0,0,0.1)', borderRadius:10, padding:'10px 14px', color:'#1e293b', fontSize:14, outline:'none', boxSizing:'border-box' as const }
  const labelStyle = { fontSize:12, color:'#475569', fontWeight:500 as const, display:'block' as const, marginBottom:6 }

  return (
    <div style={{ minHeight:'100vh', background:'#f4f6fb', display:'flex', alignItems:'center', justifyContent:'center', padding:20 }}>
      <div style={{ background:'rgba(0,0,0,0.03)', border:'1px solid rgba(0,0,0,0.08)', borderRadius:20, padding:'40px 48px', width:'100%', maxWidth:520 }}>
          ...
          <div style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:8, marginBottom:32 }}>
            {[1,2,3].map(s => (
              <div key={s} style={{ display:'flex', alignItems:'center', gap:8 }}>
                <div style={{ width:28, height:28, borderRadius:'50%', background: step >= s ? 'linear-gradient(135deg,#0891b2,#0d9488)' : 'rgba(0,0,0,0.06)', border: step >= s ? 'none' : '1px solid rgba(0,0,0,0.1)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:12, fontWeight:700, color: step >= s ? 'white' : '#94a3b8' }}>{s}</div>
                {s < 3 && <div style={{ width:40, height:2, background: step > s ? '#0891b2' : 'rgba(0,0,0,0.06)', borderRadius:2 }} />}
              </div>
            ))}
          </div>
        )}
        ...
        {step === 1 && (
          <div>
            <h2 style={{ fontSize:15, fontWeight:600, color:'#334155', marginBottom:20 }}>Personal Information</h2>
            ...
            {error && <div style={{ background:'rgba(239,68,68,0.1)', border:'1px solid rgba(239,68,68,0.2)', borderRadius:8, padding:'10px 14px', fontSize:13, color:'#dc2626', marginTop:16 }}>{error}</div>}
            <button onClick={() => {
        ...
        {step === 2 && (
          <div>
            ...
            <div style={{ display:'flex', gap:10 }}>
              <button onClick={() => { setError(''); setStep(1) }} style={{ flex:1, background:'rgba(0,0,0,0.05)', border:'1px solid rgba(0,0,0,0.1)', borderRadius:10, padding:'12px', color:'#475569', fontSize:14, cursor:'pointer' }}>← Back</button>
              <button onClick={() => {
                if (form.disciplinas.length === 0) { setError('Select at least one discipline.'); return }
                setError(''); setStep(3)
        ...
        {step === 3 && (
          <div>
            <h2 style={{ fontSize:15, fontWeight:600, color:'#334155', marginBottom:8 }}>Recording Availability</h2>
            ...
            {form.horarios.map((h, i) => (
              <div key={i} style={{ display:'flex', gap:8, marginBottom:10, alignItems:'center' }}>
                <select value={h.dia_semana} onChange={e => updateHorario(i, 'dia_semana', Number(e.target.value))}
                  style={{ background:'rgba(0,0,0,0.05)', border:'1px solid rgba(0,0,0,0.1)', borderRadius:8, padding:'9px', color:'#1e293b', fontSize:13, outline:'none' }}>
                ...
                <input type="time" value={h.hora_fim} onChange={e => updateHorario(i, 'hora_fim', e.target.value)}
                  style={{ background:'rgba(0,0,0,0.05)', border:'1px solid rgba(0,0,0,0.1)', borderRadius:8, padding:'9px', color:'#1e293b', fontSize:13, outline:'none' }} />
                <button onClick={() => removeHorario(i)} style={{ background:'rgba(239,68,68,0.1)', border:'none', borderRadius:6, padding:'8px 10px', color:'#dc2626', cursor:'pointer', fontSize:12 }}>✕</button>
              </div>
            ))}
            <button onClick={addHorario} style={{ width:'100%', background:'rgba(0,0,0,0.03)', border:'1px dashed rgba(0,0,0,0.1)', borderRadius:10, padding:'10px', color:'#64748b', fontSize:13, cursor:'pointer', marginBottom:24 }}>
              + Add time slot
            </button>
            {error && <div style={{ background:'rgba(239,68,68,0.1)', border:'1px solid rgba(239,68,68,0.2)', borderRadius:8, padding:'10px 14px', fontSize:13, color:'#dc2626', marginBottom:16 }}>{error}</div>}
            <div style={{ display:'flex', gap:10 }}>
              <button onClick={() => { setError(''); setStep(2) }} style={{ flex:1, background:'rgba(0,0,0,0.05)', border:'1px solid rgba(0,0,0,0.1)', borderRadius:10, padding:'12px', color:'#475569', fontSize:14, cursor:'pointer' }}>← Back</button>
        ...
        {step === 4 && (
          <div style={{ textAlign:'center' }}>
            ...
            <h2 style={{ fontSize:20, fontWeight:700, color:'#0f172a', marginBottom:8 }}>Registration Complete!</h2>
