...
export default function Sidebar({ session }: { session: UserSession | null }) {
  ...
  return (
    <div style={{ width: W, background: 'rgba(0,0,0,0.02)', borderRight: '1px solid rgba(0,0,0,0.06)', display: 'flex', flexDirection: 'column', position: 'fixed', top: 0, bottom: 0, left: 0, transition: 'width 0.2s ease', overflow: 'hidden', zIndex: 10 }}>
        ...
        {navItems.map(item => {
          ...
          return (
            <Link key={item.href} href={item.href} style={{ textDecoration: 'none' }}>
              <div title={!open ? item.label : ''} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '9px 12px', borderRadius: 8, color: active ? '#7c3aed' : '#64748b', background: active ? 'rgba(167,139,250,0.1)' : 'transparent', fontSize: 13, fontWeight: 500, marginBottom: 2, cursor: 'pointer', justifyContent: open ? 'flex-start' : 'center' }}>
